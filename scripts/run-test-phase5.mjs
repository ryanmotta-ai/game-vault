...
async function run() {
  console.log('[test-phase5] Compiling test suite...');
  await esbuild.build({
    entryPoints: [path.resolve(rootDir, 'scripts/test-phase5.ts')],
    outfile: path.resolve(rootDir, 'dist/test-phase5.cjs'),
    bundle: true,
    platform: 'node',
    target: 'node20',
  ...
  console.log('[test-phase5] Executing with Electron runtime...');
  const electronBin = process.platform === 'win32'
    ? path.resolve(rootDir, 'node_modules', '.bin', 'electron.cmd')
    : path.resolve(rootDir, 'node_modules', '.bin', 'electron');

  const result = spawnSync(electronBin, ['dist/test-phase5.cjs'], {
    cwd: rootDir,
    stdio: 'inherit',
    shell: true,
    env: {
