...
async function run() {
  console.log('[test-phase4c] Compiling test suite...');
  await esbuild.build({
    entryPoints: [path.resolve(rootDir, 'scripts/test-phase4c.ts')],
    outfile: path.resolve(rootDir, 'dist/test-phase4c.cjs'),
    bundle: true,
    platform: 'node',
    target: 'node20',
    external: ['better-sqlite3', 'electron', 'node-unrar-js', '7zip-bin'],
    alias: {
      ...
      '@preparation': path.resolve(rootDir, 'src/preparation'),
      '@streaming': path.resolve(rootDir, 'src/streaming')
    }
  });

  console.log('[test-phase4c] Executing with Electron runtime...');
  const electronBin = process.platform === 'win32'
    ? path.resolve(rootDir, 'node_modules', '.bin', 'electron.cmd')
    : path.resolve(rootDir, 'node_modules', '.bin', 'electron');

  const result = spawnSync(electronBin, ['dist/test-phase4c.cjs'], {
    cwd: rootDir,
    stdio: 'inherit',
    shell: true,
    env: {
